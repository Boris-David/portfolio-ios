...
/// Points at the backstage mode — **once**.
///
...
/// points at it.
package struct BackstageTip: Tip {
  ...
  package var title: Text {
    Text(language == .french ? "Les coulisses" : "The backstage")
  }
...
public enum BackstageTipState {
  ...
  ///
  /// The rule in `BackstageTip` is declarative, so this is the only call needed
  /// — there is no "dismiss" to remember from three different places.
  public static func markUsed() {
    BackstageTip.hasBeenUsed = true
  }
}
