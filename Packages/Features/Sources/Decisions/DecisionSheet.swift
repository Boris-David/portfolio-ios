    ...
    NavigationStack {
        ...
        VStack(alignment: .leading, spacing: Tokens.Space.s5) {
          ...
          if let documentation = note.documentation {
            Link(destination: documentation) {
              Label(text(DecisionLabels.documentation), systemImage: "arrow.up.right")
            }
      ...
      .toolbar {
        ToolbarItem(placement: .confirmationAction) {
          Button(text(DecisionLabels.close)) { dismiss() }
        }
      }
    }
  ...
  private var learnMore: some View {
    Button {
      ...
      HStack(spacing: Tokens.Space.s2) {
        Text(text(DecisionLabels.learnMore))
    ...
    .accessibilityHint(text(DecisionLabels.learnMoreHint))
  }
